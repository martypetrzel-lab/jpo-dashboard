...
export async function getEventsFiltered(filters, limit = 400) {
  ...
  // ✅ denní filtr pro mapu + tabulku (Europe/Prague)
  if (day === "today") {
    where.push(
...
// ✅ server očekává možnost smazat souřadnice konkrétní události podle ID
export async function clearEventCoords(id) {
  const res = await pool.query(`UPDATE events SET lat=NULL, lon=NULL WHERE id=$1`, [id]);
  return res.rowCount;
}
