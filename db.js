...
export async function initDb() {
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_events_city_text ON events (city_text);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_events_place_text ON events (place_text);`);
...
export async function upsertEvent(ev) {
  ...
  // default sanity limit (server má taky), ale DB je poslední jistota
  ...
  const q = `
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      -- end_time_iso:
      -- 1) pokud přijde z RSS platné, vezmi ho
      -- 2) jinak při přechodu OPEN->CLOSED nastav NOW() (UTC) jako ISO string
      end_time_iso = CASE
      ...
      -- duration:
      -- 1) když přijde nový duration -> vezmi ho
      -- 2) když starý duration je extrémní -> smaž ho
      -- 3) jinak drž starý
      duration_min = CASE
...
function buildWhere(filters) {
  ...
  // pracujeme nad start_time_iso (ISO text) nebo pub_date (text) fallback
  ...
  // bereme podle Europe/Prague a primárně pub_date (když je ISO), jinak created_at
  // (nejbezpečnější a stabilní pro UI filtry)
  if (filters?.day === "today" || filters?.day === "yesterday") {
    const offset = filters.day === "today" ? 0 : 1;
    // Porovnání na lokální den: created_at v DB je timestamptz → převod na Prague
    where.push(`
      DATE((created_at AT TIME ZONE 'Europe/Prague')) = (DATE(NOW() AT TIME ZONE 'Europe/Prague') - $${i}::int)
    `);
    params.push(offset);
    i++;
  }
...
export async function getEventsOutsideCz(limit = 200) {
  ...
  // hrubě "mimo ČR" pro údržbu – v praxi sem padají nesmysly z geocodu
  const q = `
    SELECT id, lat, lon
    FROM events
    WHERE lat IS NOT NULL AND lon IS NOT NULL
      AND (
        lat < 48.55 OR lat > 51.06
        OR lon < 12.09 OR lon > 18.87
      )
...
export async function updateEventDuration(id, durationMin) {
  ...
  await pool.query(`UPDATE events SET duration_min=$2 WHERE id=$1`, [id, Math.round(n)]);
}