...
function initMap() {
  ...
  map.on("click", (e) => {
    ...
    setPickMode(false);
    try { setStatus("Bod vybrán – potvrď Uložit v Admin", true); } catch {}
    // vrať admin modal, ať lze rovnou kliknout na Uložit
    try { openModal("admin"); } catch {}
    try { document.getElementById("saveCoordsBtn")?.focus(); } catch {}
  });
...
function setPickMode(on) {
  ...
  if (pickingCoords) {
    // Admin modál a backdrop překrývá mapu – pro výběr bodu musíme modály zavřít,
    // jinak klik do mapy neprojde (Leaflet klik handler se nespustí).
    closeModals();
    try { setStatus("Vyber bod na mapě…", true); } catch {}
  ...
  try {
    ...
    for (const ev of rows) {
      ...
      pickBtn.addEventListener("click", () => {
        ...
        try { document.getElementById('pickOnMapBtn')?.scrollIntoView({behavior:'smooth', block:'center'}); } catch {}
        try { document.getElementById('pickOnMapBtn')?.focus(); } catch {}
      });
