...
export async function initDb() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS events (
      ...
      first_seen_status TEXT,
      first_seen_was_open BOOLEAN,
...
export async function upsertEvent(ev) {
  ...
  await pool.query(
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      start_time_source=CASE WHEN events.start_time_source='legacy_manual_unverified' OR (events.geo_source IN ('manual_event_edit','manual_event_create') AND events.status_source IS DISTINCT FROM 'manual' AND events.source_kind IS DISTINCT FROM 'manual' AND events.duration_source IS DISTINCT FROM 'manual' AND events.start_time_source IS NULL AND (events.start_time_iso IS NOT NULL OR NULLIF(events.time_original_values->>'start_time_iso','') IS NOT NULL)) THEN 'legacy_manual_unverified' WHEN events.start_time_source IN ('manual','rss_description','explicit','esp') OR events.status_source='manual' OR events.source_kind='manual' OR events.duration_source='manual' THEN COALESCE(events.start_time_source,'manual') ELSE EXCLUDED.start_time_source END,
      ...
      start_time_iso=CASE WHEN events.start_time_source='legacy_manual_unverified' OR (events.geo_source IN ('manual_event_edit','manual_event_create') AND events.status_source IS DISTINCT FROM 'manual' AND events.source_kind IS DISTINCT FROM 'manual' AND events.duration_source IS DISTINCT FROM 'manual' AND events.start_time_source IS NULL AND (events.start_time_iso IS NOT NULL OR NULLIF(events.time_original_values->>'start_time_iso','') IS NOT NULL)) THEN COALESCE(events.start_time_iso,NULLIF(events.time_original_values->>'start_time_iso','')) WHEN events.start_time_source IN ('manual','rss_description','explicit','esp') OR events.status_source='manual' OR events.source_kind='manual' OR events.duration_source='manual' THEN events.start_time_iso WHEN EXCLUDED.source_kind='rss' THEN EXCLUDED.start_time_iso ELSE COALESCE(events.start_time_iso,EXCLUDED.start_time_iso) END,
      end_time_iso   = CASE
      ...
      duration_min=CASE WHEN events.duration_source='manual' THEN events.duration_min WHEN EXCLUDED.duration_min IS NOT NULL THEN EXCLUDED.duration_min WHEN EXCLUDED.duration_is_estimate AND EXCLUDED.duration_source='first_seen_open_estimate' THEN NULL WHEN EXCLUDED.source_kind='rss' THEN NULL ELSE events.duration_min END,
      duration_source=CASE WHEN events.duration_source='manual' THEN events.duration_source WHEN EXCLUDED.duration_source IS NOT NULL THEN EXCLUDED.duration_source WHEN EXCLUDED.source_kind='rss' THEN NULL ELSE events.duration_source END,
  ...
  if (hasMonth) {
    ...
    const longest = await pool.query(
      ...
      SELECT
        ...
        COALESCE(NULLIF(city_text,''), place_text) AS city,
        CASE
          WHEN duration_min IS NOT NULL AND duration_min > 0 AND duration_min <= $${iL} AND (COALESCE(duration_is_estimate,FALSE)=FALSE AND duration_source IN ('rss_start_and_end','manual_start_rss_end','trusted_start_rss_end','rss_end_time','esp_duration','explicit','manual') AND (source_kind IS DISTINCT FROM 'rss' OR duration_source IN ('esp_duration','explicit','manual') OR start_time_source IN ('manual','rss_description','explicit','esp') OR status_source='manual'))
            THEN duration_min
      ...
      ${whereLongestSql}
        AND (
          (duration_min IS NOT NULL AND duration_min > 0 AND duration_min <= $${iL} AND (COALESCE(duration_is_estimate,FALSE)=FALSE AND duration_source IN ('rss_start_and_end','manual_start_rss_end','trusted_start_rss_end','rss_end_time','esp_duration','explicit','manual') AND (source_kind IS DISTINCT FROM 'rss' OR duration_source IN ('esp_duration','explicit','manual') OR start_time_source IN ('manual','rss_description','explicit','esp') OR status_source='manual')))
...
export async function updateEventManualMeta(id, patch = {}) {
  ...
  await pool.query(
    ...
    SET
      ...
      end_time_iso = $9,
      duration_min = $10,
      duration_source = CASE WHEN $10::integer IS NOT NULL THEN 'manual' ELSE NULL END,
