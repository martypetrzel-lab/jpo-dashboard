...
import { buildRssPayload, readPushConfig, runRssPush } from "../scripts/rss-push.js";
...
const env = { FIREWATCH_INGEST_URL: "https://firewatchcz.cz/api/ingest", FIREWATCH_API_KEY: "test-secret-key" };
...
test("RSS connect failure makes two attempts and never calls ingest", async () => {
  ...
  assert.equal(success, false);
  assert.equal(calls, 2);
  assert.match(h.logs.join("\n"), /stage=rss; category=connect_timeout/);
  assert.equal(h.logs.join("\n").includes(env.FIREWATCH_API_KEY), false);
});
